...
export const putUpdatedPedometerData = (dayPedoOutput, userId, raceId) => async dispatch => {
  try {
    const result = await axios.get(`${server}/api/userRaces/${raceId}/${userId}`)
    const userRace = result.data[0]
    const newDifferenceFromAverage = Number(dayPedoOutput) - Number(userRace.dailyAverage)
    const newPercentageImprovement = Number(newDifferenceFromAverage)/Number(userRace.dailyAverage)
    ...
    const updatedUsers = []
    const sortedUsersArray = allUsersInRace.data.sort((user1, user2) => user2.percentImprovement - user1.percentImprovement).map(async (sortedUser, index) => {
      try{
        return await axios.put(`${server}/api/userRaces/${raceId}/${sortedUser.userId}`, 
        {
          place: index+1
        })
      } catch(err) {
